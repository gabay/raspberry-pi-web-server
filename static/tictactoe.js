...
function SetCanvasSize(){
  pageW = cvs.width = cvs.parentElement.clientWidth;
  pageH = cvs.height = cvs.parentElement.clientHeight;
...
function AIMove(board, player){
  ...
  for(i = 0; i < outcomes.squares.length; i++){
    ...
    if (testAlphaBeta > bestAlphaBeta){
      bestMoves = [outcomes.squares[i]];
      bestAlphaBeta = testAlphaBeta;
    } else if (testAlphaBeta == bestAlphaBeta) {
      bestMoves.push(outcomes.squares[i]);
    }
  }
